...
void say_spell( CHAR_DATA *ch, int sn )
{
	...
	if ( length == 0 )
	 ...
	 {
	if ( rch != ch )
	    act(AT_LBLUE, ch->class==rch->class ? buf : buf2, ch, NULL, rch, TO_VICT );
    }
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
	 act(AT_YELLOW, "$n calls God's lightning to strike $s foes!",
...
ansi_color( GREY, ch );
	...
	stop_follower( victim );
	 ...
	 affect_to_char( victim, &af );
	 act(AT_LBLUE, "Isn't $n just so nice?", ch, NULL, victim, TO_VICT );
	 if ( ch != victim )
	send_to_char( "Ok.\n\r", ch );
    return;
}
...
void spell_continual_light( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    obj_to_room( light, ch->in_room );
	 act(AT_LBLUE, "$n twiddles $s thumbs and $p appears.",   ch, light, NULL, TO_ROOM );
	 act(AT_LBLUE, "You twiddle your thumbs and $p appears.", ch, light, NULL, TO_CHAR );
	 return;
}
...
void spell_dispel_magic ( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	for ( paf = victim->affected; paf; paf = paf->next )
	{
		...
		send_to_char( "\n\r", victim );
					 act(AT_LBLUE, "$n is no longer affected by '$t'.",
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
				act(AT_LBLUE, "The white aura around $n's body fades.",
...
void spell_dispel_evil( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( IS_GOOD(victim) )
    {
		  act(AT_LBLUE, "God protects $N.", ch, NULL, victim, TO_ROOM );
		  return;
    }

    if ( IS_NEUTRAL(victim) )
    {
		  act(AT_LBLUE, "$N does not seem to be affected.", ch, NULL, victim, TO_CHAR );
		  return;
    }
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
	 act(AT_BLUE, "$n makes the earth tremble and shiver.", ch, NULL, NULL, TO_ROOM );
...
ansi_color( GREY, ch );
    ...
    obj->pIndexData = get_obj_index( 5660 );    /* So it's not a corpse */
	 ...
	 act(AT_BLUE, "Your new $p looks pretty snazzy.", ch, obj, NULL, TO_CHAR );
	 act(AT_BLUE, "$n's new $p looks pretty snazzy.", ch, obj, NULL, TO_ROOM );
...
ansi_color( GREY, ch );
	...
	REMOVE_BIT   ( ich->affected_by, AFF_SNEAK	);
		  act(AT_BLUE, "$n is revealed!", ich, NULL, NULL, TO_ROOM );
ansi_color( BOLD, ch );
ansi_color( CYAN, ch );
...
void spell_invis( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	return;
	 ...
	 act(AT_LBLUE, "$n fades out of existence.", victim, NULL, NULL, TO_ROOM );
	 af.type      = sn;
    af.duration  = 24;
    af.location  = APPLY_NONE;
    af.modifier  = 0;
...
void spell_mass_invis( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( !is_same_group( gch, ch ) || IS_AFFECTED(gch, AFF_INVISIBLE) )
	    continue;
		  act(AT_LBLUE, "$n slowly fades out of existence.", gch, NULL, NULL, TO_ROOM );
ansi_color( BOLD, ch );
ansi_color( CYAN, ch );
...
void spell_pass_door( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    affect_to_char( victim, &af );
	 act(AT_BLUE, "$n turns translucent.", victim, NULL, NULL, TO_ROOM );
ansi_color( BOLD, ch );
ansi_color( CYAN, ch );
...
void spell_shield( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.bitvector = 0;
    affect_to_char( victim, &af );
	 act(AT_LBLUE, "$n is surrounded by a force shield.", victim, NULL, NULL, TO_ROOM );
ansi_color( BOLD, ch );
ansi_color( CYAN, ch );
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
		  act(AT_LBLUE, "$n goes to sleep.", victim, NULL, NULL, TO_ROOM );
...
void spell_stone_skin( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    af.bitvector = 0;
    affect_to_char( victim, &af );
	 act(AT_DGREY, "$n's skin turns to stone.", victim, NULL, NULL, TO_ROOM );
ansi_color( BOLD, ch );
ansi_color( BLACK, ch );
...
void spell_summon( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	return;
	 ...
	 act(AT_LBLUE, "$n has been summoned away.", victim, NULL, NULL, TO_ROOM );
	 char_from_room( victim );
    char_to_room( victim, ch->in_room );
	 act(AT_LBLUE, "$n arrives suddenly.", victim, NULL, NULL, TO_ROOM );
	 act(AT_LBLUE, "$N has summoned you!", ch, NULL, victim,   TO_VICT );
	 do_look( victim, "auto" );
    return;
}
...
void spell_teleport( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	&&   !IS_SET(pRoomIndex->room_flags, ROOM_SOLITARY) )
	 ...
	 act(AT_LBLUE, "$n slowly fades out of existence.", victim, NULL, NULL, TO_ROOM );
	 char_from_room( victim );
    char_to_room( victim, pRoomIndex );
	 act(AT_LBLUE, "$n slowly fades into existence.", victim, NULL, NULL, TO_ROOM );
	 do_look( victim, "auto" );
    return;
}
...
void spell_acid_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	for ( obj_lose = ch->carrying; obj_lose != NULL; obj_lose = obj_next )
	{
		...
		{
						  act(AT_GREEN, "$p is pitted and etched!!!",   victim, obj_lose, NULL, TO_CHAR );
...
void spell_fire_breath( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	obj_lose = obj_next )
	{
	    ...
	    case ITEM_PILL:      msg = "$p melts and drips!";     break;
				...
				act(AT_RED, msg, victim, obj_lose, NULL, TO_CHAR );
				extract_obj( obj_lose );
	}
    }
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
	 act(AT_LBLUE, "$n has given $mself an adrenaline rush!", ch, NULL, NULL,
		  TO_ROOM );
...
void spell_awe ( int sn, int level, CHAR_DATA *ch, void *vo )
  {
	...
	stop_fighting ( victim, TRUE);
		  act(AT_LBLUE, "$N is in AWE of you!", ch, NULL, victim, TO_CHAR    );
		  act(AT_LBLUE, "You are in AWE of $n!",ch, NULL, victim, TO_VICT    );
		  act(AT_LBLUE, "$N is in AWE of $n!",  ch, NULL, victim, TO_NOTVICT );
	 }
    return;
}
...
void spell_ballistic_attack ( int sn, int level, CHAR_DATA *ch, void *vo )
  {
    ...
    if ( saves_spell( level, victim ) )
      dam /= 2;
	 act(AT_LBLUE, "You chuckle as a stone strikes $N.", ch, NULL, victim,
	TO_CHAR );
	 damage( ch, victim, dam, sn);
    return;
}
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
	 act(AT_LBLUE, "$n is surrounded by a white aura.", victim, NULL, NULL, TO_ROOM );
	 return;
}
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
		  act(AT_LBLUE, "$N looks better.", ch, NULL, victim, TO_NOTVICT );
	 }
    if ( is_affected( victim, gsn_curse  ) )
    {
	affect_strip( victim, gsn_curse  );
...
void spell_combat_mind ( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( victim == ch )
	  ...
	  act(AT_BLOOD, "$N already understands battle tactics.",
	      ch, NULL, victim, TO_CHAR );
	return;
    }
...
//	      continue;
	  ...
	  if ( number_bits( 2 ) != 0 )
	      continue;
			 ...
			 act(AT_LBLUE, "$p disintegrates!",      victim, obj_lose, NULL, TO_CHAR );
			 act(AT_LBLUE, "$n's $p disintegrates!", victim, obj_lose, NULL, TO_ROOM );
			 extract_obj( obj_lose ) ;
      }
     ...
     */
    {
		  act(AT_LBLUE, "You have DISINTEGRATED $N!",         ch, NULL, victim, TO_CHAR );
		  act(AT_LBLUE, "You have been DISINTEGRATED by $n!", ch, NULL, victim, TO_VICT );
		  act(AT_LBLUE, "$n's spell DISINTEGRATES $N!",       ch, NULL, victim, TO_ROOM );
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
	 act(AT_LBLUE, "$N shimmers and appears in a different location.",
	ch, NULL, victim, TO_NOTVICT );
	 return;
}
...
void spell_domination ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    affect_to_char( victim, &af );
	 ...
	 act(AT_LBLUE, "Your will dominates $N!", ch, NULL, victim, TO_CHAR );
	 act(AT_LBLUE, "Your will is dominated by $n!", ch, NULL, victim, TO_VICT );
	 return;
}
...
void spell_ego_whip ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    affect_to_char( victim, &af );
	 ...
	 act(AT_LBLUE, "You ridicule $N about $S childhood.", ch, NULL, victim, TO_CHAR    );
ansi_color( BOLD, ch );
ansi_color( CYAN, ch );
	 send_to_char( "Your ego takes a beating.\n\r", victim );
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
	 act(AT_LBLUE, "$N's ego is crushed by $n!",          ch, NULL, victim, TO_NOTVICT );
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
	 act(AT_LBLUE, "$N's flesh turns to steel.", ch, NULL, victim, TO_NOTVICT);
	 return;
}
...
void spell_inertial_barrier ( int sn, int level, CHAR_DATA *ch, void *vo )
{
	...
	if ( !is_same_group( gch, ch ) || IS_AFFECTED( gch, AFF_PROTECT ) )
	    continue;
		  ...
		  act(AT_LBLUE, "An inertial barrier forms around $n.", gch, NULL, NULL,
		 TO_ROOM );
ansi_color( BOLD, ch );
ansi_color( CYAN, ch );
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
		  act(AT_LBLUE, "A virtual fortress forms around $N.", gch, NULL, gch, TO_ROOM );
...
void spell_lend_health ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( hpch == 0 )
    {
	act(AT_LBLUE, "Nice thought, but $N doesn't need healing.", ch, NULL,
	    victim, TO_CHAR );
	return;
    }
    if ( ch->hit-hpch < 50 )
    ...
    update_pos( ch );
	 ...
	 act(AT_LBLUE, "You lend some of your health to $N.", ch, NULL, victim, TO_CHAR );
	 act(AT_LBLUE, "$n lends you some of $s health.",     ch, NULL, victim, TO_VICT );
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
	 act(AT_LBLUE, "$n's feet rise off the ground.", victim, NULL, NULL, TO_ROOM );
	 return;
}
...
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
	 act(AT_LBLUE, "$n appears drained of strength.", victim, NULL, NULL, TO_ROOM );
	 return;
}
...
void spell_share_strength ( int sn, int level, CHAR_DATA *ch, void *vo )
{
    ...
    if ( is_affected( victim, sn ) )
    {
	act(AT_LBLUE, "$N already shares someone's strength.", ch, NULL, victim,
	    TO_CHAR );
	return;
    }
    ...
    affect_to_char( ch,     &af );
	 ...
	 act(AT_LBLUE, "You share your strength with $N.", ch, NULL, victim, TO_CHAR );
	 act(AT_LBLUE, "$n shares $s strength with you.",  ch, NULL, victim, TO_VICT );
	 return;
}
...
ansi_color( GREY, ch );
          ...
          */
          ...
          char_to_room( undead , ch->in_room );
			 act(AT_LBLUE, "$p staggers to its feet.", undead, vobj, NULL, TO_ROOM);
    ...
    if (f)
		 act(AT_RED, "You carefully draw a pentagram around nothing, and nothing happens.",
          ch, NULL, NULL, TO_CHAR );
}
