...
void weather_update( void )
{
	...
	weather_info.sunlight = SUN_RISE;
		  strcat( buf, "The sun peeks above the skyline to the east.\n\r" );
		  break;

	 case 19:
	weather_info.sunlight = SUN_SET;
		  strcat( buf, "The sun disappears below the horizon to your west.\n\r" );
		  break;
...
void char_update( void )
{
	...
	if ( !IS_NPC(ch) && ch->level < LEVEL_IMMORTAL )
	{
		...
		{
		    --ch->in_room->light;
						  act(AT_GREY, "$p goes out.", ch, obj, NULL, TO_ROOM );
						  act(AT_GREY, "$p goes out.", ch, obj, NULL, TO_CHAR );
						  extract_obj( obj );
		}
	    }
		...
		if ( ch->was_in_room == NULL && ch->in_room != NULL )
		{
			...
			stop_fighting( ch, TRUE );
						  act(AT_PURPLE, "$n's limp body disappears into the void.",
...
{   
	...
	if ( obj->timer <= 0 || --obj->timer > 0 )
				...
				if (obj->in_room &&
					...
					if (( rch = obj->in_room->people ) != NULL )
						{
						act(AT_BLUE, "$p falls away.", rch, obj, NULL, TO_ROOM );
						act(AT_BLUE, "$p falls away.", rch, obj, NULL, TO_CHAR );
						}
               ...
               if (( rch = obj->in_room->people ) != NULL )
                  {
						act(AT_BLUE, "$p floats by.", rch, obj, NULL, TO_ROOM );
						act(AT_BLUE, "$p floats by.", rch, obj, NULL, TO_CHAR );
						}
               }
#endif
	    continue;
	...
	{
				act(AT_BLOOD, message, obj->carried_by, obj, NULL, TO_CHAR );
		  }
	else if ( obj->in_room != NULL
	&&      ( rch = obj->in_room->people ) != NULL )
	{
		 act(AT_BLOOD, message, rch, obj, NULL, TO_ROOM );
		 act(AT_BLOOD, message, rch, obj, NULL, TO_CHAR );
	}
