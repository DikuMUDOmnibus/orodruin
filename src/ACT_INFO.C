...
void show_char_to_char_1( CHAR_DATA *victim, CHAR_DATA *ch )
{
	 ...
	 {
	act(AT_WHITE, "$n looks at you.", ch, NULL, victim, TO_VICT    );
	act(AT_WHITE, "$n looks at $N.",  ch, NULL, victim, TO_NOTVICT );
	 }
...
ansi_color( GREY, ch );
	 ...
	 {
	act(AT_WHITE, "You see nothing special about $M.", ch, NULL, victim, TO_CHAR );
	 }
	...
	&&   can_see_obj( ch, obj ) )
	{
		 ...
		 {
		send_to_char( "\n\r", ch );
		act(AT_RED, "$N is using:", ch, NULL, victim, TO_CHAR );
		found = TRUE;
		 }
                 ansi_color( BOLD, ch );
                 ansi_color( BLUE, ch );
...
void do_look( CHAR_DATA *ch, char *argument )
{
	...
	case ITEM_CORPSE_PC:
		...
		break;
		 ...
		 act(AT_RED, "$p contains:", ch, obj, NULL, TO_CHAR );
	...
	{
	    act(AT_RED, "The $d is open.",   ch, NULL, pexit->keyword, TO_CHAR );
	}
    }
...
void do_who( CHAR_DATA *ch, char *argument )
{
	...
	case MAX_LEVEL - 1: class = " Deity "; break;
	case MAX_LEVEL - 2: class = " Saint "; break;
	case MAX_LEVEL - 3: class = " Angel "; break;
	case MAX_LEVEL - 4: class = "  Hero "; break;
	}

	if ( wch->pcdata->clan )
	{
	  strcpy( clan_name, " (" );
	  if ( !str_cmp( wch->name, wch->pcdata->clan->deity ) )
	    strcat( clan_name, "Deity of " );
	  else
	  if ( !str_cmp( wch->name, wch->pcdata->clan->leader ) )
	    strcat( clan_name, "Leader of " );
	  else
	  if ( !str_cmp( wch->name, wch->pcdata->clan->number1 ) )
	    strcat( clan_name, "Number One " );
	  else
	  if ( !str_cmp( wch->name, wch->pcdata->clan->number2 ) )
	    strcat( clan_name, "Number Two " );
	  strcat( clan_name, wch->pcdata->clan_name );
	  strcat( clan_name, ")" );
	}
	else
	  clan_name[0] = '\0';
...
{   sprintf( buf + strlen(buf), "[%s] %2d %s  %s%s%s%s.%s\n\r",
		 race_table[wch->race].who_name,
		 wch->level,
		 class,
		 IS_SET(wch->act, PLR_KILLER) ? "(KILLER) " : "",
...
void do_compare( CHAR_DATA *ch, char *argument )
{
    ...
    act(AT_WHITE, msg, ch, obj1, obj2, TO_CHAR );
    return;
}
...
void do_consider( CHAR_DATA *ch, char *argument )
{
    ...
    diff = victim->level - ch->level;
	 ...
	 act(AT_RED, msg, ch, NULL, victim, TO_CHAR );
...
void do_practice( CHAR_DATA *ch, char *argument )
{
	...
	else
	{
		 ...
		 if ( ch->pcdata->learned[sn] < adept )
	    {
					 act(AT_BLUE, "You practice $T.",
			 ch, NULL, skill_table[sn].name, TO_CHAR );
					 act(AT_BLUE, "$n practices $T.",
								 ch, NULL, skill_table[sn].name, TO_ROOM );
					  }
		 else
		 {
		ch->pcdata->learned[sn] = adept;
					 act(AT_BLUE, "You are now an adept of $T.",
			 ch, NULL, skill_table[sn].name, TO_CHAR );
					 act(AT_BLUE, "$n is now an adept of $T.",
			 ch, NULL, skill_table[sn].name, TO_ROOM );
					  }
	}
	 }
...
void do_channels( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( !IS_SET(ch->deaf, CHANNEL_SPEW)
		 ? " +SPEW"
		 : " -spew",
		 ch );
	...
	else
	{
		 ...
		 return;
		  ...
		  else if ( !str_cmp( arg+1, "spew"    ) )  bit = CHANNEL_SPEW;
...
ansi_color( YELLOW, ch );
        ...
        send_to_char(  IS_SET(ch->act, PLR_ANSI)
		 ? "[+ANSI     ] You see ansi color.\n\r"
		 : "[-ansi     ] You don't see ansi color.\n\r"
		 , ch );
ansi_color( NTEXT, ch );
ansi_color( GREY, ch );
