...
void move_char( CHAR_DATA *ch, int door )
{
    ...
    {
				  act(AT_WHITE, "The $d is closed.", ch, NULL, pexit->keyword, TO_CHAR );
		          return;
    }
	...
	{
            ansi_color( BOLD, ch );
            send_to_char( "You are too exhausted!  Better rest for a while.\n\r", ch );
            ansi_color( NTEXT, ch );
            return;
	}
...
{		  act(AT_RED, "$n leaves $T.", ch, NULL, dir_name[door], TO_ROOM );
		   }
    char_from_room( ch );
    ...
    && ( IS_NPC(ch) || !IS_SET(ch->act, PLR_WIZINVIS) ) )
{        
		  act(AT_RED, "$n has arrived.", ch, NULL, NULL, TO_ROOM );
}
	...
	{
				act(AT_PURPLE, "You follow $N.", fch, NULL, ch, TO_CHAR );
				move_char( fch, door );
	}
    }
...
int find_door( CHAR_DATA *ch, char *arg )
{
    ...
    if ( ( pexit = ch->in_room->exit[door] ) == NULL )
    {
	act(AT_WHITE, "I see no door $T here.", ch, NULL, arg, TO_CHAR );
	return -1;
    }
...
void do_open( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "Ok.\n\r", ch );
		  act(AT_RED, "$n opens $p.", ch, obj, NULL, TO_ROOM );
		  return;
    }
	...
	act(AT_RED, "$n opens the $d.", ch, NULL, pexit->keyword, TO_ROOM );
	send_to_char( "Ok.\n\r", ch );
...
{					 act(AT_RED, "The $d opens.", rch, NULL, pexit_rev->keyword, TO_CHAR );
}
        }
    }
...
void do_close( CHAR_DATA *ch, char *argument )
{
	...
	send_to_char( "Ok.\n\r", ch );
	act(AT_RED, "$n closes $p.", ch, obj, NULL, TO_ROOM );
	return;
    }
	...
	SET_BIT(pexit->exit_info, EX_CLOSED);
		  act(AT_RED, "$n closes the $d.", ch, NULL, pexit->keyword, TO_ROOM );
		  send_to_char( "Ok.\n\r", ch );
	...
	&&   pexit_rev->to_room == ch->in_room )
	{
	    ...
	    for ( rch = to_room->people; rch != NULL; rch = rch->next_in_room )
{					 act(AT_RED, "The $d closes.", rch, NULL, pexit_rev->keyword, TO_CHAR );
}
        }
    }
...
void do_lock( CHAR_DATA *ch, char *argument )
{
	...
	SET_BIT(obj->value[1], CONT_LOCKED);
        ...
        send_to_char( "*Click*\n\r", ch );
		  ansi_color( NTEXT, ch );
		  ansi_color( GREY, ch );
		  act(AT_RED, "$n locks $p.", ch, obj, NULL, TO_ROOM );
		  return;
    }
	...
	SET_BIT(pexit->exit_info, EX_LOCKED);
        ...
        ansi_color( RED, ch );        
		  ...
		  ansi_color( NTEXT, ch );
		  ansi_color( GREY, ch );
	act(AT_RED, "$n locks the $d.", ch, NULL, pexit->keyword, TO_ROOM );
	/* lock the other side */
	if ( ( to_room   = pexit->to_room               ) != NULL
	&&   ( pexit_rev = to_room->exit[rev_dir[door]] ) != 0
	&&   pexit_rev->to_room == ch->in_room )
...
void do_unlock( CHAR_DATA *ch, char *argument )
{
	...
	REMOVE_BIT(obj->value[1], CONT_LOCKED);
        ...
        send_to_char( "*Click*\n\r", ch );
		  ansi_color( NTEXT, ch );
		  ansi_color( GREY, ch );
	act(AT_RED, "$n unlocks $p.", ch, obj, NULL, TO_ROOM );
		  return;
    }
	...
	REMOVE_BIT(pexit->exit_info, EX_LOCKED);
        ...
        send_to_char( "*Click*\n\r", ch );
		  ansi_color( NTEXT, ch );
		  ansi_color( GREY, ch );
	act(AT_RED, "$n unlocks the $d.", ch, NULL, pexit->keyword, TO_ROOM );
...
void do_pick( CHAR_DATA *ch, char *argument )
{
	...
	{
	    act(AT_RED, "$N is standing too close to the lock.",
		ch, NULL, gch, TO_CHAR );
	    return;
	}
    }
	...
	REMOVE_BIT(obj->value[1], CONT_LOCKED);
        ...
        send_to_char( "*Click*\n\r", ch );
		  ansi_color( NTEXT, ch );
		  ansi_color( GREY, ch );
	act(AT_RED, "$n picks $p.", ch, obj, NULL, TO_ROOM );
		  return;
    }
	...
	REMOVE_BIT(pexit->exit_info, EX_LOCKED);
		  ...
		  ansi_color( NTEXT, ch );
		  ansi_color( GREY, ch );
	act(AT_RED, "$n picks the $d.", ch, NULL, pexit->keyword, TO_ROOM );
	/* pick the other side */
	if ( ( to_room   = pexit->to_room               ) != NULL
	&&   ( pexit_rev = to_room->exit[rev_dir[door]] ) != NULL
	&&   pexit_rev->to_room == ch->in_room )
...
void do_stand( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_AFFECTED(ch, AFF_SLEEP) )
		 { send_to_char( "You can't wake up!\n\r", ch ); return; }
		  ...
		  act(AT_GREEN, "$n wakes and stands up.", ch, NULL, NULL, TO_ROOM );
	...
	break;
	 ...
	 case POS_RESTING:
		  ...
		  ansi_color( GREEN, ch );
		  send_to_char( "You wake and stand up.\n\r", ch );
		  act(AT_GREEN, "$n stands up.", ch, NULL, NULL, TO_ROOM );
		  ch->position = POS_STANDING;
	break;
...
void do_rest( CHAR_DATA *ch, char *argument )
{
	...
	break;
	 ...
	 case POS_STANDING:
		  ...
		  act(AT_GREEN, "$n rests.", ch, NULL, NULL, TO_ROOM );
...
void do_sleep( CHAR_DATA *ch, char *argument )
{
	...
	break;
	 ...
	 case POS_STANDING:
		  ...
		  ansi_color( NTEXT, ch );
		  ansi_color( GREY, ch );
		  act(AT_GREEN, "$n sleeps.", ch, NULL, NULL, TO_ROOM );
...
void do_wake( CHAR_DATA *ch, char *argument )
{
	...
	{ send_to_char( "They aren't here.\n\r",              ch ); return; }
	 ...
	 if ( IS_AWAKE(victim) )
	{ act(AT_WHITE, "$N is already awake.", ch, NULL, victim, TO_CHAR ); return; }

	 if ( IS_AFFECTED(victim, AFF_SLEEP) )
	{ act(AT_WHITE, "You can't wake $M!",   ch, NULL, victim, TO_CHAR );  return; }

	 act(AT_GREEN, "You wake $M.", ch, NULL, victim, TO_CHAR );
	 act(AT_GREEN, "$n wakes you.", ch, NULL, victim, TO_VICT );
...
void do_recall( CHAR_DATA *ch, char *argument )
{
    ...
    ROOM_INDEX_DATA *location;
	 ...
	 act(AT_WHITE, "$n prays for transportation!", ch, 0, 0, TO_ROOM );
...
ansi_color( GREY, ch );
    ...
    ch->move /= 2;
	 act(AT_PURPLE, "$n disappears.", ch, NULL, NULL, TO_ROOM );
	 char_from_room( ch );
	 char_to_room( ch, location );
	 act(AT_PURPLE, "$n appears in the room.", ch, NULL, NULL, TO_ROOM );
	 do_look( ch, "auto" );
...
void do_identify( CHAR_DATA *ch, char *argument )
{
    ...
    if (IS_IMMORTAL(ch))
       act(AT_WHITE, "$n cackles and comps you!\n\r", rch, obj, ch, TO_VICT );
    else if (ch->gold < obj->level * 100 + 50)
       {
       act(AT_WHITE, "$n resumes tossing bones without looking at $p.",
             rch, obj, 0, TO_ROOM );
       return;
       }
    else
    ...
    act(AT_WHITE, "$n fondles $p and tosses some chicken bones to the ground.",
       rch, obj, 0, TO_ROOM );
...
void do_bash ( CHAR_DATA *ch, char *argument )
{
	...
	{
		 act(AT_RED, "WHAAAAM!!!  You bash against the $d, but it doesn't budge.",
		ch, NULL, pexit->keyword, TO_CHAR );
		 act(AT_RED, "WHAAAAM!!!  $n bashes against the $d, but it holds strong.",
		 ...
		 && number_percent( ) < ( chance + 4 * ( get_curr_str( ch ) - 20 ) ) )
	{
		 ...
		 act(AT_RED, "Crash!  You bashed open the $d!", ch, NULL, pexit->keyword, TO_CHAR );
		 act(AT_RED, "$n bashes open the $d!",          ch, NULL, pexit->keyword, TO_ROOM );
	...
	else
	{
		 ...
		 act(AT_RED, "OW!  You bash against the $d, but it doesn't budge.",
		ch, NULL, pexit->keyword, TO_CHAR );
		 act(AT_RED, "$n bashes against the $d, but it holds strong.",
		ch, NULL, pexit->keyword, TO_ROOM );
